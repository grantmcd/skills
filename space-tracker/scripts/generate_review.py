...
def generate_review_data(agency_filter=None, search_filter=None):
    ...
    if agency_filter:
        results = [
            ...
            if agency_filter.lower()
            in launch.get("launch_service_provider", {}).get("name", "").lower()
        ]
    if search_filter:
        results = [
            launch
            for launch in results
            if search_filter.lower() in launch.get("name", "").lower()
            or search_filter.lower()
            in launch.get("mission", {}).get("description", "").lower()
        ]
        ...
        launches.append(
            {
                ...
                if launch.get("mission")
                else "No mission details available.",
    ...
    if os.path.exists(TEMPLATE_PATH):
        ...
        final_html = template.replace(
            "const data = {{DATA}};",
            f"const data = {json.dumps(review_data, indent=4)};",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate review data for the Space Tracker UI."
    )
