...
## Instructions
1. **Fetch Data:** When triggered, run the `scripts/fetch_launches.py` script to retrieve the latest launch data from the Launch Library 2 API.
...
6. **Generate Review:** Once data is fetched, run the `scripts/generate_review.py` script to prepare the JSON payload for the Review UI.
7. **Display UI:** Open the `ui/viewer.html` file using the generated data to show the user a visual dashboard of upcoming launches.
