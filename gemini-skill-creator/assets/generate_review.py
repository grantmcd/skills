...
def generate_review(workspace_dir, output_file):
    ...
    with_skill_results = next((r for r in benchmark["results"] if r["config_name"] == "with_skill"), {})
    html = html.replace("{{pass_rate}}", str(int(with_skill_results.get("pass_rate", 0) * 100)))
